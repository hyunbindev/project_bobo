...
import {
  AwardCard,
  type AwardTheme,
} from "@/components/clan-awards/award-card";
import { BoboKingCard } from "@/components/clan-awards/bobo-king-card";
import type { RegularAwardCode } from "@/lib/awards/types";
import { getMainClanSummary } from "@/lib/services/clan-service";
import { getWeeklyAwardPageData } from "@/lib/services/weekly-award-service";

export const metadata: Metadata = {
  title: "최근 일주일 랭킹 | BOBO",
  description: "이번 주 클랜 경기 기록으로 선정한 BOBO 클랜 칭호와 순위",
};

// 아이콘과 색상은 UI 설정이므로 서비스가 아닌 화면 계층에서 관리한다.
const awardThemes: Record<RegularAwardCode, AwardTheme> = {
  boost: {
    icon: Pill,
    accent: "text-primary",
    glow: "bg-primary/8",
...
export default async function AwardPage() {
  ...
  const data = await getWeeklyAwardPageData(mainClan?.id ?? null);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
        ...
        <div className="relative mx-auto grid max-w-360 gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-12 lg:py-24">
          <div>
            <p className="mb-4 flex items-center gap-2 text-[10px] font-black tracking-[0.27em] text-primary">
              <Sparkles className="size-3.5" /> WEEKLY HONORS
            </p>
          ...
          <div className="grid grid-cols-2 gap-px self-end overflow-hidden rounded-sm border border-border/60 bg-border/60">
            <HeroMetric
              label="AWARDS"
              value={String(data.awards.length).padStart(2, "0")}
            />
      ...
      <section className="mx-auto max-w-360 px-5 py-14 sm:px-8 lg:px-12 lg:py-20">
        ...
        <BoboKingCard award={data.boboKing} />
      ...
      <footer className="border-t border-border/50 bg-surface">
        <div className="mx-auto flex max-w-360 flex-col gap-3 px-5 py-8 text-[9px] font-semibold tracking-wide text-muted-foreground sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-12">
          <span>BOBO CLAN · WEEKLY HONORS</span>
